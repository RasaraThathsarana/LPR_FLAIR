...
def load_checkpoint(
    ...
    # Robust checkpoint decoding accounting for LPR or standard main_decoders
    if "refiner_head.3.weight" in model_dict or "model.refiner_head.3.weight" in model_dict:
        weight_key = "model.refiner_head.3.weight" if "model.refiner_head.3.weight" in model_dict else "refiner_head.3.weight"
        bias_key = weight_key.replace("weight", "bias")
        n_classes_total = sum(len(conf["labels_configs"][t]["value_name"]) for t in tasks)
        check_and_reinit_layer(state_dict, model_dict, weight_key, bias_key, n_classes_total,
                            matched_tasks, reinit_tasks, "LPR_Refiner_Head", reinit_counter)
    else:
        for task in tasks:
            weight_keys = [
